...
class ViewController: UIViewController {
    ...
    func loaderFunction()  {
        ...
        flagPlaceButton.setTitle("Tap for flag", for: .normal)
        time = 0
        taps = 0
        timer.invalidate()
    ...
    @IBOutlet weak var flagPlaceButton: UIButton!
    
    @IBOutlet weak var timerLabel: UILabel!
    
    @IBOutlet weak var wordsLabel: UILabel!
    
    @IBAction func flagPlaceButtonTap(_ sender: UIButton) {
        if sender.title(for: .normal) == "Continue playing" {
            sender.setTitle("Tap for flag", for: .normal)
        } else if sender.title(for: .normal) == "Tap for flag" {
            sender.setTitle("Continue playing", for: .normal)
        }
        if flag == true {
